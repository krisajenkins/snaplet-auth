    ...
    case maybeAccount of
      ...
      Just (account, accountUidpwd) ->
        let resetToken =
            ...
            toAddress =
              Address Nothing (unEmail (accountUidpwdEmail accountUidpwd))
            mail =
...
makeResetEmail config toAddress passwordResetRequest resetToken = Mail {..}
  where
    mailFrom =
      Address
...
processPasswordResetCompletion = do
  ...
  runMaybeT $ do
    uuid <-
      MaybeT . pure $ extractVerifiedResetUUID secretKey passwordResetCompletion
    hashedPassword <- MaybeT (hashRequestedPassword passwordResetCompletion)
...
emailPasswordResetCompletionHandler =
  method POST $ do
    maybeAccount <- processPasswordResetCompletion
    case maybeAccount of
