...
lookupByUsername username =
  ...
  (select . from $
   \(account `InnerJoin` accountUidpwd) ->
     do on $
          account ^. AccountAccountId ==. accountUidpwd ^.
          AccountUidpwdAccountId
...
data HashedPassword = HashedPassword {hash ::Text}

hashFor :: Text -> IO (Maybe HashedPassword)
hashFor password =
  do hashed <- hashPasswordUsingPolicy fastBcryptHashingPolicy $ encodeUtf8 password
     return (HashedPassword . decodeUtf8 <$> hashed)

createUidpwdUser :: UUID -> UTCTime -> Text -> HashedPassword -> SqlPersistM (Key Account)
createUidpwdUser uuid created username hashedPassword =
  do accountKey <- insert $ Account uuid created
     _ <-
